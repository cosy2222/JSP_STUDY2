...
<CENTER>
 ...
 <% 	// vector : 멀티스레드 환경에서 사용 , 모든 메소드가 동기화 처리되어 있음. 
  Vector name=new Vector();		 // DB의 name 값을 저장하는 벡터
  Vector inputdate=new Vector();
  Vector email=new Vector();
  Vector subject=new Vector();
  Vector rcount=new Vector();
  
  Vector step=new Vector();	      // DB의 step컬럼의 값을 저장하는 벡터
  ...
  int maxpages=3;   // 페이지 갯수 ( 화면에 출력될 페이지갯수 ) 
  ...
  int wheregroup=1;			// 현재 위치하는 그룹
  	...
  	// freeboard_list.jsp?gogroup=2   (maxpage가 5일떄  6,7,8,9,10)
  	
  	// go 변수 (페이지 번호)를 넘겨 받아서 wheregroup , startpage , endpage 정보의 값을 알아낸다 
  	
  if (request.getParameter("go") != null) {			// go 변수의 값을 가지고 있을떄
   where = Integer.parseInt(request.getParameter("go"));   // 현재 페이지 번호를 담은 변수 
   wheregroup = (where-1)/maxpages + 1;			// 현재 위치한 페이지의 그룹 
   startpage=(wheregroup-1) * maxpages+1;  
   endpage=startpage+maxpages-1; 
   
   // gogroup 변수를 넘겨 받아서 startpage , endpage , where (페이지 그룹의 첫번쨰 페이지)
  } else if (request.getParameter("gogroup") != null) {	 // gogroup 변수의 값을 가지고 올떄 
  ...
  int startrow=0;   // 하나의 페이지 에서 Select한 레코드 시작 번호 
  int endrow=0;		// 하나의 페이지 에서 Select한 레코드 마지막 번호 
  int maxrows=3;      //출력할 레코드 수 
  int totalrows=0;		// 총 레코드 갯수 
  int totalpages=0;		// 총 페이지 갯수 
  
//  out.println("======== maxpage가 3일떄 ===========" + "<p>");
...
//  if (true) return;   // 프로그램 종료 
  ...
  if (!(rs.next()))  {
   ...
   do {
	   // DateBase의 값을 가져와서 각각의 Vector에 저장 
...
//   out.println("토탈 페이지 그룹 " + totalgroup + "<p>");   
   ...
   // for 블락 시작  : 현재 페이지에서 시작레코드 , 마지막레코드 까지 순환하면서 출력 
   for(int j=startrow;j<=endrow;j++) {
    String temp=(String)email.elementAt(j);    // email vector에서  email 주소를 가져온다 
    if ((temp == null) || (temp.equals("")) ) 		// 메일주소가 비어있을떄
